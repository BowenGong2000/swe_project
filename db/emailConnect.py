...
def email_send(reciver, application_dict):
    ...
    msg['Subject'] = Header('Application Recieved', 'utf-8')
    ...
    resume = application_dict[RESUME_FILENAME]
    transcript = application_dict[TRANSCRIPT_FILENAME]
    cover_l = application_dict[COVERLETTER_FILENAME]

    if RESUME_FILENAME:
        attachment = MIMEApplication(application_dict[RESUME_CONTENT],
                                     _subtype=mimetypes.guess_type(resume))
    ...
    if TRANSCRIPT_FILENAME:
        attachment = MIMEApplication(application_dict[TRANSCRIPT_CONTENT],
                                     _subtype=mimetypes.guess_type(transcript))
        attachment.add_header('content-disposition',
                              'attachment',
    ...
    if COVERLETTER_FILENAME:
        attachment = MIMEApplication(application_dict[COVERLETTER_CONTENT],
                                     _subtype=mimetypes.guess_type(cover_l))
        attachment.add_header('content-disposition',
                              'attachment',
